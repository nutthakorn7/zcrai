...
describe('Profile Controller', () => {
    ...
    it('should get profile', async () => {
        const { data, response } = await api.profile.index.get({ headers })
    ...
    it('should update profile', async () => {
        const { data, response } = await api.profile.index.put({
            name: 'Updated Name',
            jobTitle: 'Tester',
            bio: 'Automated Bio'
        }, { headers })
        ...
        // Verify persistence
        const { data: current } = await api.profile.index.get({ headers })
        expect(current?.name).toBe('Updated Name')
    })

    it('should list active sessions', async () => {
        const { data, response } = await api.profile.sessions.index.get({ headers })
        expect(response.status).toBe(200)
